 ...
 * the letters and vowel signs it is built from. Devanagari writes syllables
 * rather than letters, so this is the bridge between reading the romanization
        ...
        <div className="rounded-xl border border-line bg-surface-1/80 p-3">
          ...
          <ul className="divide-y divide-line/60">
            {aksaras[open].parts.map((p, k) => (
              <li key={`${p.char}-${k}`} className="flex items-center gap-3 py-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap items-baseline gap-x-2">
                    ...
                    <span className="text-[11px] text-ink-faint">{ROLE_LABEL[p.role] ?? p.role}</span>
                  </div>
                  <div className="mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-ink-muted">
                    ...
                    {p.inherentVowel && <span className="text-gold">carries inherent “a”</span>}
                  </div>
                </div>
              </li>
            ))}
